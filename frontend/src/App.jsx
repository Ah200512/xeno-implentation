      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: '1280px', margin: '0 auto', padding: '2rem 1.5rem' }}>
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'results' && validationResult && (
            ...
            >
              {/* Main Title Header */}
              <header style={{ display: 'flex', alignItems: 'center', gap: '0.85rem', marginBottom: '2.5rem' }}>
                <img src="/newlogoxeno.png" alt="Xeno Logo" style={{ height: '30px', objectFit: 'contain' }} />
                <div style={{ borderLeft: '1px solid rgba(255, 255, 255, 0.15)', paddingLeft: '0.85rem' }}>
                  <h2 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '1.25rem', fontWeight: 700, letterSpacing: '-0.02em', color: '#ffffff', display: 'flex', alignItems: 'center', gap: '6px', lineHeight: '1.2' }}>
                    Integrity Engine
                  </h2>
                  <span style={{ fontSize: '0.65rem', color: '#94a3b8', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginTop: '2px' }}>Automated AI Clearing</span>
                </div>
              </header>

              {/* Action Buttons Header - exactly the 3 things */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
                <div style={{ display: 'flex', gap: '0.75rem', position: 'relative' }}>
                  ...
                  <motion.button
                    ...
                    className="premium-btn"
                    style={{
                      background: showTable ? 'var(--primary-glow-strong)' : 'rgba(255, 255, 255, 0.03)',
                      borderColor: showTable ? 'var(--primary)' : 'var(--card-border)',
                      color: showTable ? 'var(--primary)' : 'var(--text-secondary)',
                      fontWeight: showTable ? 600 : 500
                    }}
                  >
                      ...
                      {showSplitPopover && (
                        ...
                        >
                          ...
                          >
                            ...
                            {splitting ? 'Generating...' : 'Download ZIP'}
                          </motion.button>
                        </motion.div>
                      )}
                    </AnimatePresence>
                ...
                {/* File summary details in header right */}
                <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', textAlign: 'right' }}>
                  <div>Source: <strong style={{ color: 'var(--text-primary)' }}>{file?.name}</strong></div>
                  <div>Size: <strong style={{ color: 'var(--text-primary)' }}>{(file?.size / 1024).toFixed(1)} KB</strong></div>
                </div>
              </div>
